...
using Wave_T    = BAEMM::Helmholtz_OpenCL::WaveType;
...
//    constexpr Int wave_count = 32;
    ...
    H.FarField<wave_count>(
        ...
        F.data(),
        Wave_T::Plane, cg_tol, gmres_tol
    );
    toc("FarField");
    
    print("");
    
    std::ofstream((home_dir / "F_True.txt").string()) << ToString(F);
    
    Real factor = Frac<Real>(2 * Scalar::Pi<Real>, meas_count * wave_count );

    Real norm = std::sqrt(factor) * F.FrobeniusNorm();

    std::cout << "L^2-norm of F = " << norm << std::endl;
    ...
    H.Derivative_FF<wave_count>(
        kappa.data(), wave_chunk_count,
        inc.data(),   wave_chunk_size,
        h.data(), DFh.data(), &du_dn, Wave_T::Plane, cg_tol, gmres_tol
    );
    toc("Derivative_FF");
    
    std::ofstream((home_dir / "DF_True.txt").string()) << ToString(DFh);
    ...
    H.AdjointDerivative_FF<wave_count>(
        kappa.data(), wave_chunk_count,
        inc.data(),   wave_chunk_size,
        DFh.data(), w.data(), &du_dn, Wave_T::Plane, cg_tol, gmres_tol
    );
    toc("AdjointDerivative_FF");
    
    std::ofstream((home_dir / "DFT_True.txt").string()) << ToString(w);

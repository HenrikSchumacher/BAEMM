...
// FIXME: xcrun -sdk macosx metal -c GEMM_CM_C.metal -o GEMM_CM_C.air && xcrun -sdk macosx metallib GEMM_CM_C.air -o GEMM_CM_C.metallib
...
[[kernel]] void GEMM_CM_C(
    constant const uint  & M                    [[buffer(0)]],
    constant const uint  & N                    [[buffer(1)]],
    constant const uint  & K                    [[buffer(2)]],
    constant const cmplx & alpha                [[buffer(3)]],
    constant const cmplx * const A              [[buffer(4)]],
    constant const cmplx * const B              [[buffer(5)]],
    constant const cmplx & beta                 [[buffer(6)]],
    device         cmplx * const C              [[buffer(7)]],
...
//    const uint threadId = (g_rows * g_cols) * groupId + g_rows * l_j + l_i;
    ...
    threadgroup cmplx A_shared[tile_K][tile_M];
    threadgroup cmplx B_shared[tile_N][tile_K];
                    ...
                    if( k_complete )
                    {
                        for( uint t_k = 0; t_k < tile_K; ++t_k )
                        {
                            ...
                            for( uint t_j = 0; t_j < 4; ++t_j )
                            {
                                for( uint t_i = 0; t_i < 4; ++t_i )
                                {
                                    C_tile[t_j][t_i][0] +=    A_t_k[t_i][0] * B_t_k[t_j][0]
                                                            - A_t_k[t_i][1] * B_t_k[t_j][1];
                                    C_tile[t_j][t_i][1] +=    A_t_k[t_i][0] * B_t_k[t_j][1]
                                                            + A_t_k[t_i][1] * B_t_k[t_j][0];
                                }
                            }
                        }
                    }

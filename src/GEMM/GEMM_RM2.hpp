public:
    ...
    )
    {
        MTL::ComputePipelineState * pipeline = GetPipelineState(
              "GEMM_RM",
              std::string(
#include "GEMM_RM.metal"
              ),
              {"int","int"},
              {"SIMD_width","SIMD_log"},
              {ToString(32),ToString(5)}
          );
        
        assert( pipeline != nullptr );
        assert( pipeline->threadExecutionWidth() == 32 );
        ...
        compute_encoder->setBytes(&M,          sizeof(int),   0);
        compute_encoder->setBytes(&N,          sizeof(int),   1);
        compute_encoder->setBytes(&K,          sizeof(int),   2);
...
//        const NS::Integer max_threads = pipeline->maxTotalThreadsPerThreadgroup();
        ...
        MTL::Size threadgroups_per_grid  (
            CeilDivide(N, 4 * static_cast<Int>(threads_per_threadgroup.width  )),
            CeilDivide(M, 4 * static_cast<Int>(threads_per_threadgroup.height )),
            1
        );
